...
public record Mkdir<T extends FileSystem>(Tree<T> tree, String name, TreeNode<T> currentNode)
    implements Command<T>, CreatorCommand {
  ...
  public Result<T> execute() {
    if (isValidName(name)) {
      ...
      return createResult(
          newTree.withCurrentNode(newTree.findNode(currentNode.getData())), message);
    } else {
